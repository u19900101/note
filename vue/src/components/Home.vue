...
export default {
  ...
  methods: {
    ...
    synchronous() {
      ...
      let changeObj = this.$store.state.noteModule.currentNotes.filter(item => item.id == this.EditId)[0];
      ...
      if (changeObj) {
        ...
        if (this.EditId !== '' && (this.EditTitle != changeObj.title || this.EditTextarea != changeObj.content)) {
          ...
          // 更新 noteModule中的数据
          this.$store.dispatch('noteEditChange',params)
          // 更新数据库
          //  发送post请求 修改数据库文件
          this.https.noteEditChange(params).then(({data}) => {
    ...
    getData() {
      ...
      this.https.getNotebooks().then(({data}) => {
        ...
        this.https.getNotes().then(({data}) => {
          this.$store.dispatch('getNotes', data.data);
        }).then(() => {
    ...
    inteContent() {
      ...
      this.noteContent = currentNoteToShow;
      this.titleValue = currentNoteToShow.title;
      this.textareaValue = currentNoteToShow.content;
  ...
  // 钩子函数 请求数据同步vuex
  created() {
    // 不使用 localstorage 存储，改为使用mysql
    // 使用异步获取 笔记本和笔记数据
    console.log("mmmm")
    this.getData();


    // this.$store.state.noteModule.currentNotes = this.$store.state.allList;
    // let id = this.$route.params.id;
    //
    // //防止胡乱修改路由动态id
    // let n = this.$store.state.noteModule.currentNotes.some(item => item.id == id);
    // if (n) {
    //   this.inteContent();
    // } else {
    //   this.$router.push({
  ...
  watch: {
    ...
    titleValue(newTitle) {
      ...
      this.EditId = this.$route.params.id || this.$store.state.noteModule.currentNotes[0].id.toString(); // 字符串类型的id
    },
    // 监听textarea内容
    textareaValue(newTextarea) {
      this.EditTextarea = newTextarea;
      this.EditId = this.$route.params.id || this.$store.state.noteModule.currentNotes[0].id.toString();
    },
